...
public final class StringUtils {
    ...
    public static String checkPluginVersion(String version) {
        String[] items = version.split("\\.");
        /**
         * {major}.{minor}.{revise}[-dev | release | bugfix ]
         */
        if (items.length != 3) {
            return "expect version format: {major}.{minor}.{revise}[-dev | -release | -bugfix ]\n example:" +
                    " 1.0.0-dev or 1.0.0";
        }

        int index = version.lastIndexOf("-");
        if (index >= 0) {
            String suffix = version.substring(index + 1);

            if (!"dev".equals(suffix)
                    && !"release".equals(suffix)
                    && !"bugfix".equals(suffix)) {
                return "plugin version should ends with '-dev'、'-release' or '-bugfix'";
            }

            // update last element : revision
            int i = items[items.length - 1].indexOf("-");
            items[items.length - 1] = items[items.length - 1].substring(0, i);
        }
        ...
        for (String i : items) {
            try {
                ...
                return "version contains invalid number: '" + i + "'";
            }
        }
        return null;
    }
